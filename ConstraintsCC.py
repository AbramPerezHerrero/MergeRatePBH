...
-This program serve to obtain the value of the maximum black hole abundance for 
 a Critical Collapse mass function and for values of \beta=[2.85,1.7,1.2] 
 parameter.
...
def mergeRate(Mi,Mj,f,N,Mc1,beta1):
...
def check(vec,Mc2,I,beta1):
            ...
            for Mi in mass_1:
                def mergeRatePLBf(Mj):
                    return mergeRate(Mi,Mj,f,I,Mc2,beta1)   
...
names=['CC17','CC12','CC285',]
# A counter variable
h=0
# First loop for each value of parameter beta
for beta1 in [1.7,1.2,2.85]:
    namefile=names[h]
    h=h+1
        ...
        if(j==30):
            ...
            for f in v[j]:  #Third loop for each value of parameter f in the 
                ...
                for Mi in mass_1:# Four loop for each value of M1 
                        def mergeRatef(Mj):
                            return mergeRate(Mi,Mj,f,I,Mc1,beta1)   
