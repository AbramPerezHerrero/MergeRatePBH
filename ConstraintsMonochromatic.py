...
-This program serve to obtain the value of the maximum black hole abundance for 
 a pseudo-monochromatic mass function
 parameter.

@Return: A txt file with the values of  maximum f_{PBH} for each value of M0
...
def Monochromatic(M,I,M01):
    ...
    '''
    return MF.logNormal(M,A=I,sigma=0.05,Mc=M01)
...
Mmax=100;
# Array with the values of M0
M0values=np.linspace(6,95,45)
#The array fval is used to save the value of f_{pbh} for each value of Mc.
fval=np.zeros([len(M0values)])
...
def check(vec,M02,I):
            ...
            v: Array
                ...
                to check.
            M02 : FLOAT
                M0 value of the mass function parameter.
            I : FLOAT
                Normalization of the mass function.
            ...
            for Mi in mass_1:
                def mergeRatePLBf(Mj):
                    return mergeRate(Mi,Mj,f,I,M02)   
...
namefile='Monochromatic'


# Second loop for each value of parameter Mc
for M01 in M0values:
        print('M0:',M01) # Only if you want to show in which step are analised
        # Obtain the normalization of the mass function
        I=MF.normalization("monochromatic",sigma=0.05,Mc=M01)
        #time counter
        start_time = time()
        #step counter
        k=0
