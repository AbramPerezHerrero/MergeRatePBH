...
-This program serve to obtain the value of the maximum black hole abundance for 
 a Critical Collapse mass function and for values of \beta=[2.85] 
 parameter and clustering parameter [10,100]
...
def critical(M,I,Mc1,beta1):
    ...
    beta1: FLOAT
        beta parameter of the CC mass function
    Returns
    -------
    FLOAT
    ...
    '''
    return MF.criticalCollapse(M,A=I,beta=beta1,Mc=Mc1)

# This function represent the equation 3.58 of the work
def mergeRate(Mi,Mj,f,N,Mc1,beta1,zeta):
    ...
    '''
    # In this case the function used is critical
    function=critical 
    if Mj!=Mi:
            return (3.7e6)*(f*0.85)**2*zeta**(16/37)*((f*0.85)**2+(0.005)**2)**(-21/74)*(Mi+Mj)**(36/37)*(Mi*Mj)**(3/37)*min(function(Mi,N,Mc1,beta1)/Mi,function(Mj,N,Mc1,beta1)/Mj)*(function(Mi,N,Mc1,beta1)/Mi+function(Mj,N,Mc1,beta1)/Mj)
    else :
            return (3.7e6)*(f*0.85)**2*zeta**(16/37)*((f*0.85)**2+(0.005)**2)**(-21/74)*(Mi+Mj)**(36/37)*(Mi*Mj)**(3/37)*(function(Mj,N,Mc1,beta1)/(2*Mj))*(function(Mj,N,Mc1,beta1)/(Mj))
...
Mcvalues=np.concatenate((np.linspace(0.3,3,25)[0:25],np.linspace(3,100,25)[0:24]
                         ,np.linspace(100,800,27)))
#The array fval is used to save the value of f_{pbh} for each value of Mc.
fval=np.zeros([len(Mcvalues)])

def check(vec,Mc2,I,beta1,zeta1):
            '''
            ...
            for Mi in mass_1:
                def mergeRatePLBf(Mj):
                    return mergeRate(Mi,Mj,f,I,Mc2,beta1,zeta1)   
...
for zeta1 in [10,100]:
    ...
    for Mc1 in Mcvalues:
        ...
        I=MF.normalization("criticalCollapse",beta=beta1,Mc=Mc1)
        #time counter
        start_time = time()
        #step counter
        k=0
        ...
        for i in np.arange(0,15,1):
            if check(v[i],Mc1,I,beta1,zeta1):
                j=i
        ...
        if(j==30):
            ...
            for f in v[j]:  #Third loop for each value of parameter f in the 
                ...
                for Mi in mass_1:# Four loop for each value of M1 
                        def mergeRatef(Mj):
                            return mergeRate(Mi,Mj,f,I,Mc1,beta1,zeta1)   
