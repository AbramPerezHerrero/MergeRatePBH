...
def mergeRate(Mi,Mj,f,N,delta1,alpha1,zeta):
...
Mcvalues=np.concatenate((np.linspace(3.1,100,36)[0:35]
                         ,np.linspace(100,800,30)))
#The array fval is used to save the value of f_{pbh} for each value of Mc.
fval=np.zeros([len(Mcvalues)])

def check(vec,delta2,I,alpha1,zeta1):
            alpha11=2.7
            '''
            ...
            for Mi in mass_1:
                def mergeRatePLBf(Mj):
                    return mergeRate(Mi,Mj,f,I,delta2,alpha1,zeta1)   
...
for zeta1 in [10,100]:
    ...
    for delta1 in Mcvalues:
        ...
        I=MF.normalization("powerLaw",alpha=2.7,delta=delta1)
        #time counter
        start_time = time()
        #step counter
        k=0
        ...
        for i in np.arange(0,15,1):
            if check(v[i],delta1,I,2.7,zeta1):
                j=i
        ...
        if(j==30):
            ...
            for f in v[j]:  #Third loop for each value of parameter f in the 
                ...
                for Mi in mass_1:# Four loop for each value of M1 
                        def mergeRatef(Mj):
                            return mergeRate(Mi,Mj,f,I,delta1,2.7,zeta1)   
