...
def mergeRate(Mi,Mj,f,N,Mc1,sigma1):
...
Mcvalues=np.concatenate((np.linspace(0.1,1,15)[0:15-1],
                         np.linspace(1,95,25)[0:24],np.linspace(95,500,15)[0:14],
                         np.linspace(450,800,18)))    
#The array fval is used to save the value of f_{pbh} for each value of Mc.
fval=np.zeros([len(Mcvalues)])

def check(vec,Mc2,I,sigma1):
            '''
            ...
            for Mi in mass_1:
                def mergeRatePLBf(Mj):
                    return mergeRate(Mi,Mj,f,I,Mc2,sigma1)   
...
names=['Monocromatic','LN04','LN062','LN1']
# A counter variable
h=0
# First loop for each value of parameter sigma
for sigma1 in [0.05,0.4,0.6,1]:
    namefile=names[h]
    h=h+1
    ...
    for Mc1 in Mcvalues:
        ...
        #I=MF.normalization("LogNormal",sigma=sigma1,Mc=Mc1)
        I=MF.normalization("monochromatic",sigma=sigma1,Mc=Mc1)
        #time counter
        start_time = time()
        #step counter
        k=0
        ...
        if(j==30):
            ...
            for f in v[j]:  #Third loop for each value of parameter f in the 
                ...
                for Mi in mass_1:# Four loop for each value of M1 
                        def mergeRatef(Mj):
                            return mergeRate(Mi,Mj,f,I,Mc1,sigma1)   
